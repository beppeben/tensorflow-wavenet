...
from .ops import causal_conv, mu_law_encode, text_to_ints
...
class WaveNetModel(object):
    ...
    def __init__(self,
                 ...
                 local_condition_gaussians=None):
        ...
        Args:
            ...
            dilations_lc: A list with the dilation factor for each layer,
                to be applied after the local conditioning has been computed.
            ...
            local_condition_cardinality: Size of character dictionary for 
                local condition.
            local_condition_channels: Number of channels in (embedding
                size) of local conditioning characters. None indicates there is
                no local conditioning.
            local_condition_gaussians: Number of gaussians to be used for
                computing local conditioning weights.

        '''
        self.batch_size = batch_size
        self.dilations = dilations
        self.dilations_lc = dilations_lc
        ...
        self.local_condition_gaussians = local_condition_gaussians

        self.receptive_field = WaveNetModel.calculate_receptive_field(
            self.filter_width, self.dilations, self.dilations_lc, self.scalar_input,
            self.initial_filter_width)
        self.variables = self._create_variables()

    @staticmethod
    def calculate_receptive_field(filter_width, dilations, dilations_lc, scalar_input,
                                  initial_filter_width):
        receptive_field = (filter_width - 1) * (sum(dilations) + sum(dilations_lc))  + 1
        if scalar_input:
    ...
    def _create_dilation_variables(self, with_local=False):
        current = dict()
    ...
    def _create_variables(self):
        ...
        with tf.variable_scope('wavenet'):
            ...
            if self.local_condition_cardinality is not None:
                with tf.variable_scope('embeddings'):
                    layer = dict()
                    if 'embeddings' in var:
                        layer = var['embeddings']
            ...
            with tf.variable_scope('dilated_stack'):
                for i, dilation in enumerate(self.dilations):
                    with tf.variable_scope('layer{}'.format(i)):
                        current = self._create_dilation_variables()
            ...
            with tf.variable_scope('dilated_stack_lc'):
                for i, dilation in enumerate(self.dilations_lc):
                    with tf.variable_scope('layer{}'.format(i)):
                        current = self._create_dilation_variables(with_local=True)
                        var['dilated_stack_lc'].append(current)                    
    ...
    def _create_dilation_layer(self, input_batch, layer_index, dilation,
        ...
        skip_cut = tf.shape(conv_gate)[1] - output_width
                                      
        if local_condition_batch is not None:
            local_padded = tf.pad(local_condition_batch, [[0, 0], [skip_cut, 0], [0, 0]])
            weights_lc_filter = variables['lc_filtweights']
            conv_filter = conv_filter + tf.nn.conv1d(local_padded,
                                                     weights_lc_filter,
                                                     stride=1,
                                                     padding="SAME",
                                                     name="lc_filter")
            weights_lc_gate = variables['lc_gateweights']
            conv_gate = conv_gate + tf.nn.conv1d(local_padded,
                                                 weights_lc_gate,
                                                 stride=1,
                                                 padding="SAME",
                                                 name="lc_gate")
    ...
    def _generate_lc_layer(self, input_batch, local_condition_batch, init_ops, push_ops):
        ...
        q = tf.FIFOQueue(
            1,
            dtypes=tf.float32,
            shapes=(self.batch_size, self.local_condition_gaussians))
        init = q.enqueue_many(
            tf.zeros((1, self.batch_size,
                      self.local_condition_gaussians)))
        current_kappascumul = q.dequeue()
        kappascumul = current_kappascumul + kappas
        push = q.enqueue(kappascumul)
        ...
        return local_cond, weights

    def _create_lc_layer(self, input_batch, local_condition_batch):
        if local_condition_batch is None:
            return None
        lc_weights = self.variables['lc_layer']['lc_gaussian_weights']
        if self.use_biases:
            lc_biases = self.variables['lc_layer']['lc_gaussian_biases']
        if self.histograms:
            tf.histogram_summary('gaussian_weights', lc_weights)
            if self.use_biases:
                tf.histogram_summary('gaussian_biases', lc_biases)
        transformed1 = tf.nn.relu(input_batch)
    ...
    def predict_proba_incremental(self, waveform, global_condition=None,
        ...
        with tf.name_scope(name):
            ...
            raw_output, weights = self._create_generator(encoded, gc_embedding, lc_embedding)
            out = tf.reshape(raw_output, [-1, self.quantization_channels])
            proba = tf.cast(
                tf.nn.softmax(tf.cast(out, tf.float64)), tf.float32)
            last = tf.slice(
                ...
                [tf.shape(proba)[0] - 1, 0],
                [1, self.quantization_channels])
            return tf.reshape(last, [-1]), weights
